...
#define m68k_read_immediate_16(address) *(uint16_t *)(m68ki_cpu.memory_map[((address)>>16)&0xff].base + ((address) & 0xffff))
 ...
 */
static const uint16_t m68ki_exception_cycle_table[256] =
{
...
INLINE uint32_t m68ki_read_16(uint32_t address)
{
  ...
  else val = *(uint16_t *)(temp->base + ((address) & 0xffff));
...
static const uint32_t m68ki_cycle_interrupts[10] = {50*MUL, 59*MUL, 58*MUL, 57*MUL, 56*MUL, 55*MUL, 54*MUL, 53*MUL, 52*MUL, 51*MUL};
