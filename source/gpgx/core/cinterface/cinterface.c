...
int gpgx_init()
{
	...
	bitmap.height = 512;
	bitmap.pitch = 1024 * 4;
	bitmap.data = malloc(2 * 1024 * 1024);
	tempsram = malloc(24 * 1024);
	bg_pattern_cache = malloc(0x80000);

	ext.md_cart.rom = malloc(32 * 1024 * 1024);
	SZHVC_add = malloc(131072);
	SZHVC_sub = malloc(131072);
	ym2612_lfo_pm_table = malloc(131072);
	vdp_bp_lut = malloc(262144);
	vdp_lut = malloc(6 * sizeof(*vdp_lut));
	for (int i = 0; i < 6; i++)
		vdp_lut[i] = malloc(65536);
	...
	config.region_detect = 0; /* AUTO */ // settings->Region; // see loadrom.c
	...
	cinterface_custom_backdrop_color = 0;
